...
public class SQLite3Provider
    extends SQLProviderAdapter<SQLite3GlobalState, SQLite3Options> {
    ...
    public enum Action implements AbstractAction<SQLite3GlobalState> {
        ...
        @Override
        public SQLQueryAdapter getQuery(SQLite3GlobalState state)
            throws Exception {
    ...
    @Override
    public void generateDatabase(SQLite3GlobalState globalState)
        throws Exception {
        ...
        if (globalState.getDbmsSpecificOptions().generateDatabase) {
            ...
            StatementExecutor<SQLite3GlobalState, Action> se =
            ...
            SQLQueryAdapter query = SQLite3TransactionGenerator.generateCommit(
                globalState
            );
            globalState.executeStatement(query);

            // also do an abort for DEFERRABLE INITIALLY DEFERRED
            query = SQLite3TransactionGenerator.generateRollbackTransaction(
                globalState
            );
            globalState.executeStatement(query);
        }
    ...
    private SQLQueryAdapter getTableQuery(
        SQLite3GlobalState globalState,
        int i
        ...
        switch (Randomly.fromList(options)) {
            ...
            case FTS:
                ...
                tableQuery =
                    SQLite3CreateVirtualFTSTableGenerator.createTableStatement(
                        ftsTableName,
            ...
            case RTREE:
                ...
                tableQuery =
                    SQLite3CreateVirtualRtreeTabelGenerator.createTableStatement(
                        rTreeTableName,
                        globalState
                    );
                break;
    ...
    @Override
    public SQLConnection createDatabase(SQLite3GlobalState globalState)
        throws SQLException {
        ...
        if (
            dataBase.exists() &&
            ((SQLite3GlobalState) globalState).getDbmsSpecificOptions()
                .deleteIfExists
        ) {
            dataBase.delete();
        }
    ...
    public String getQueryPlan(
        ...
        SQLQueryAdapter q = new SQLQueryAdapter(
            SQLite3ExplainGenerator.explain(selectStr),
            errors
        );
    ...
    @Override
    protected void executeMutator(int index, SQLite3GlobalState globalState)
        throws Exception {
    ...
    @Override
    protected boolean addRowsToAllTables(SQLite3GlobalState globalState)
        throws Exception {
        List<SQLite3Table> tablesNoRow = globalState
            .getSchema()
            .getDatabaseTables()
            .stream()
        ...
        for (SQLite3Table table : tablesNoRow) {
            SQLQueryAdapter queryAddRows = SQLite3InsertGenerator.insertRow(
                globalState,
                table
            );
